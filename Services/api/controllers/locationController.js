...
exports.list_all_continents = function(req, res) {
    var result =[];
    axios.get('https://api.teleport.org/api/continents/').then(resp => {
    if(resp.data){
  ...
  exports.list_all_countries = function(req, res) {
    var result =[];
    var url ;
    console.log(req.params.continentId);
    if(req.params.continentId){
     url = `https://api.teleport.org/api/${req.params.continentId}/countries/`;
    } else{
 ...
 exports.list_all_cities = function(req, res) {
    ...
    if(req.params.continentId){
     url = `https://api.teleport.org/api/urban_area/${req.params.countryId}/cities/`;
    } else{
     url = 'https://api.teleport.org/api/urban_areas/';
    }
    axios.get(url).then(resp => {
    if(resp.data){
        _.map(resp.data._links['city:items'],i=>{
            var splitted = i.href.split('/');
            result.push({name:i.name,id:splitted[5]})
        })
        res.json(result);
